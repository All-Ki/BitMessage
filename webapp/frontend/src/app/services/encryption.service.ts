...
export abstract class EncryptionService {
  ...
  static async generateRSAKeyPairFromPrivateKey(priv_key: string) {
    ...
    if (priv_key.startsWith('0x')) {
      //remove 0x so we keep maximum entropy
    ...
    console.log(
      'hashing took approx ' +
        hash_total_time / iterations +
        'ms per hash, total time : ' +
        hash_total_time +
        'ms'
    );
    //TODO : RISK ASSESMENT
  ...
  static key_to_storage(keypair: any) {
    return JSON.stringify({
      private_key: forge.pki.privateKeyToPem(keypair.private_key),
      public_key: forge.pki.publicKeyToPem(keypair.public_key),
    });
  }
  static key_from_storage(key: any) {
    key = JSON.parse(key);
    return {
      private_key: forge.pki.privateKeyFromPem(key.private_key),
      public_key: forge.pki.publicKeyFromPem(key.public_key),
    };
  }
  ...
  public static async encryptDataSymmetric(
    data: string,
    key: string
  ): Promise<string> {
    const encoder = new TextEncoder();
  ...
  public static async decryptDataSymmetric(
