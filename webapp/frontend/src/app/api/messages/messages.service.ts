...
import { DiscussionLight } from ':common/models';
...
export class MessagesService extends ServiceWithInit {
  ...
  override async OnStorageReady() {
    ...
    let discussions = await this.storage.get<Map<string, Message[]>>(
      'discussions'
    );
    if (discussions != null) {
  ...
  async postMessage(text: string, receiver: string) {
    ...
    const headers = this.userSvc.buildHeaders(
      CONSTANTS.Actions.send_message,
      nonce
    );
    ...
    await ApiService.post(environment.url + '/messages', msg, {
      headers: headers,
    });
  ...
  async getMessagesFrom(from: string): Promise<Message[]> {
    ...
    const headers = this.userSvc.buildHeaders(
      CONSTANTS.Actions.get_messages,
      nonce
    );
    const res = await ApiService.get<any>(
      environment.url + '/messages/' + this.current_user + '/' + from,
      { headers: headers }
    );
    let msgs = res.data;
  ...
  messageListToDiscussion(messages: Message[]): DiscussionLight {
    if (messages.length == 0) {
      return new DiscussionLight();
    }
    let discussion: DiscussionLight = new DiscussionLight();
    discussion.me = this.userSvc.getCurrentUser();
    discussion.other =
      messages[0].sender == discussion.me
        ? messages[0].receiver
        : messages[0].sender;
  ...
  async getDiscussions(): Promise<any> {
    ...
    await this.WaitUntilReady();
    let discussions: DiscussionLight[] = [];
